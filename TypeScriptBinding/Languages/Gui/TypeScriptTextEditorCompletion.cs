...
{	
	...
	public class TypeScriptTextEditorCompletion : CompletionTextEditorExtension
	{
		...
		private TypeScriptParameterDataProvider parameterDataProvider;
		...
		private void FetchCompletionData (CodeCompletionContext completionContext)
		{
			...
			if (completionContext != null && completionContext.TriggerOffset != mCacheTriggerOffset)
			{
				...
				string data = TypeScriptCompilerManager.GetCompletionData (Document.Project, mTempBaseDirectory, mTempFileName, mCacheTriggerOffset);
			...
			if (mCanRunCompletion)
			{
				...
				if (mCompletionEnabled)
				{
					...
					if (!mCacheIsObject && mCacheXML != null && completionContext.TriggerLine == mCacheTriggerLine)
					{
						if (parameterDataProvider == null)
						{
							parameterDataProvider = new TypeScriptParameterDataProvider ();
		...
		public override void Initialize ()
		{
			...
			if (Document.HasProject && Document.Project is TypeScriptProject)
			{
