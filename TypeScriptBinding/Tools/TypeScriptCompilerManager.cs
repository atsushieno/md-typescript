...
namespace MonoDevelop.TypeScriptBinding.Tools
{
	...
	static class TypeScriptCompilerManager
	{
		...
		private static BuildError CreateErrorFromString (TypeScriptProject project, string text)
		{
			...
			if (Int32.TryParse (match.Result ("${column}"), out n))
				error.Column = n+1; //TypeScript counts zero based
			else
				error.Column = -1;
		...
		public static string GetCompletionData (Project project, string classPath, string fileName, int position)
		{
			...
			{
				PropertyService.Set ("TypeScriptBinding.EnableCompilationServer", true);
				PropertyService.Set ("TypeScriptBinding.CompilationServerPort", 6000);
			...
			if (PropertyService.Get<bool> ("TypeScriptBinding.EnableCompilationServer")) {
				
				var port = PropertyService.Get<int> ("TypeScriptBinding.CompilationServerPort");
		...
		static BuildResult ParseOutput (TypeScriptProject project, string stderr)
		{
		...
		static void ParserOutputFile (TypeScriptProject project, BuildResult result, StringBuilder output, string filename)
		{
		...
		public static bool CanRun (TypeScriptProject project, TypeScriptProjectConfiguration configuration, ExecutionContext context)
		{
		...
		public static void Run (TypeScriptProject project, TypeScriptProjectConfiguration configuration, IProgressMonitor monitor, ExecutionContext context)
		{
			ExecutionCommand cmd = CreateExecutionCommand (project, configuration);
		...
		private static void StartServer ()
		{
			...
			compilationServerPort = PropertyService.Get<int>("TypeScriptBinding.CompilationServerPort");
			compilationServer = new Process ();
			compilationServer.StartInfo.FileName = "TypeScript";
