...
namespace Mono.JavaScript.Node.Debugger
{
	class NodeBacktrace: IBacktrace, IObjectValueSource
	{
		...
		public StackFrame[] GetStackFrames (int firstIndex, int lastIndex)
		{
			...
			NodeCommandResult res = session.RunCommand ("-stack-list-frames", firstIndex.ToString (), lastIndex.ToString ());
		...
		public ObjectValue[] GetLocalVariables (int frameIndex, EvaluationOptions options)
		{
			...
			NodeCommandResult res = session.RunCommand ("-stack-list-locals", "0");
		...
		public ObjectValue[] GetParameters (int frameIndex, EvaluationOptions options)
		{
			List<ObjectValue> values = new List<ObjectValue> ();
			SelectFrame (frameIndex);
			NodeCommandResult res = session.RunCommand ("-stack-list-arguments", "0", frameIndex.ToString (), frameIndex.ToString ());
		...
		ObjectValue CreateVarObject (string exp)
		{
			try {
				session.SelectThread (threadId);
				exp = exp.Replace ("\"", "\\\"");
				NodeCommandResult res = session.RunCommand ("-var-create", "-", "*", "\"" + exp + "\"");
		...
		public AssemblyLine[] Disassemble (int frameIndex, int firstLine, int count)
		{
			...
			if (buffer == null) {
				...
				buffer = new NodeDissassemblyBuffer (session, addr);
	...
	class NodeDissassemblyBuffer: DissassemblyBuffer
	{
		NodeDebuggerSession session;
		
		public NodeDissassemblyBuffer (NodeDebuggerSession session, long addr): base (addr)
		{
			this.session = session;
		}
